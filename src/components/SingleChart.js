...
var bgColor = [];
var bdColor = [];
const [GrowthValueData,setGrowthValueData] = useState({
  labels: tempLabels,
  datasets: [
    {
...
const [FinalValueData,setFinalValueData] = useState({
  labels: tempLabels,
  datasets: [
    {
      backgroundColor:bgColor,
  ...
  axios.get(url)
    .then((response)=>{
      ...
      for(var j = 0; j < response.data[0].data.datasets.length; j++){
        ...
        var growthValue = (response.data[0].data.datasets[j].data[response.data[0].data.datasets[j].data.length-1]-response.data[0].data.datasets[j].data[0] )/response.data[0].data.datasets[j].data[0] ;
        tempGrowthData.push(growthValue*100);
        response.data[0].data.datasets[j].borderColor=colors[j];
        response.data[0].data.datasets[j].backgroundColor=opacityRgba(colors[j]);
...
const [dougChartOptions,setDougChartOptions] = useState({
  ...
  const [FinalValueOptions,setFinalValueOptions] = useState({
    scales:{
      x: {
        ticks:{
          color:"white",
    ...
    if(obj.op === "WRITE"){
      ...
      if(location_state.name === obj.response.file){
        ...
        for(var k = 0; k < obj.response.data.datasets.length; k++){
          newBarData.push(obj.response.data.datasets[k].data[obj.response.data.datasets[k].data.length-1]);
        }
        setFinalValueData({
          labels: tempLabels,
          datasets: [
    ...
    <div>
      ...
      <div data-aos="fade-right" data-aos-delay="500" data-aos-duration="1000" className="InfoContainer">
        ...
        <div className="BarContainer">
          ...
          <Chart type="bar" data={FinalValueData} options={FinalValueOptions}/>
          <h1 style={{color:"white"}}>Growth Values in %</h1>
          <Chart type="bar" data={GrowthValueData} options={FinalValueOptions}/>
        </div>
        </div>
      </div>
    </div>
